...
function animaCss() {
  return src("node_modules/animate.css/animate.css").pipe(dest(path.dist.css))
}

function jss() {
  return src("src/scripts/main.js").pipe(dest(path.dist.js))
}
...
function vendorJS() {
  ...
  return src(modules).pipe(dest("dist/scripts"))
}

function vendorCSS() {
  const modules = ["node_modules/swiper/swiper-bundle.min.css"]

  return src(modules).pipe(dest("dist/styles"))
}
